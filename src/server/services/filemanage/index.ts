...
import { ApiStore } from '@/server/modules/ApiStore';
...
export class ApiService {
  ...
  // 开启工作流
    ...
    try {
    await this.ApiStore.workflowsRun(fileId);
    } catch(err) {
  ...
  getAssistantList = async (locale: Locales): Promise<DiscoverAssistantItem[]> => {
    const json = await this.ApiStore.getAgentIndex(locale, revalidate);
  ...
  getAssistantById = async (
    ...
    let res = await fetch(this.ApiStore.getAgentUrl(identifier, locale), {
      next: { revalidate: 12 * revalidate },
    });

    if (!res.ok) {
      res = await fetch(this.ApiStore.getAgentUrl(DEFAULT_LANG), {
        next: { revalidate: 12 * revalidate },
      });
    }
